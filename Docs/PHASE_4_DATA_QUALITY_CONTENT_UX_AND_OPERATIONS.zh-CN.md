...
状态：4A–4C 已完成；4D 运行时整合完成、本地重发待继续；4K Cloudflare R2 迁移按用户要求暂停（2026-07-31）。
...
| 4D 运行时整合与重发 | 正式跨语言组进入 Catalog，详情和图鉴真实可切换 | 正式资料抽样；单语言隐藏；收藏数量不合并 | 进行中：运行时完成 |
| 4E Catalog v2 玩家元数据 | 通用 package kind、语言、名称、日期、世代、排序和依赖 | schema 安全解析；537 包迁移；安装收据身份稳定 | 待开始 |
| 4F 内容库与下载队列 | 虚拟化列表、筛选排序、批量选择、有限并发和网络策略 | 2,000 包压力测试；537 包实测；暂停/恢复/空间不足 | 待开始 |
| 4G 规则与玩法深化 | 数据驱动配列、证据标签、批量开包、历史与统计 | 所有产品显式分类；事务保存；动画/音效完整 | 待开始 |
...
- 正式本机 Catalog 验证为 147/147 来源组均生成运行时语言组，44,076 个来源 Item、53,296 个 Printing；错误的英文/简中同号候选仍为两个单语言项。图鉴 Card→Subject 生成器同步使用 `语言 + Set + 来源卡 ID + 卡号` 唯一键，不再回退到旧碰撞键。
- 收藏详情继续按稳定 Printing ID 读取拥有数；语言切换不修改全局卡片语言或应用语言。语言按钮与卡图使用 120 ms 淡入，减少动态效果时即时更新，切换继续播放翻卡音效；只有一个已安装语言时不显示切换器。
- 定向运行时测试 4/4、正式 Catalog 3/3、图鉴关联 3/3、收藏 PlayMode 2/2、完整 EditMode 363/363、完整 PlayMode 10/10 通过。本切片未构建 APK；4D 下一步是把精简覆盖文件纳入新的本地内容寻址包，重建并回读 Catalog，但不上传 Site 或 R2。
...
当前下一步为 4C 卡图补齐与容量优化。4K R2 迁移保持暂停；除非用户以后明确恢复，不要求 R2 凭据，也不执行上传、远端配置或删除。
