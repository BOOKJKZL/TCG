...
状态：4A–4J、4L 与 4M 本机静态范围已完成，schema v2/revision 6 的 538 包已发布到正式 Site 只读入口并通过独立公网审计，完成度为 96%；4H 真实邮箱等待外部 Client ID，4I 实体机长时验收待执行，4K Cloudflare R2 迁移按用户要求继续暂停（2026-08-01）。
...
3. 让 538 个及未来更多内容包可以按卡牌语言、世代、卡包名称、编号、日期和安装状态顺畅管理。
...
| 4M 最终验收 | 新 Catalog、最终 ARM64 APK、自动化与设备收据 | 新定义下 `PROJECT COMPLETION VERIFIED: 100%` | 远端验收完成（96%）；实体 Android 验收 4% 待执行 |
...
#### 4I 本机完成记录（2026-07-31）

- `CardTextureCache` 已从单一张数限制升级为“张数 + 解码 bytes”双门槛；默认每个缓存最多 48 MiB，以 `width × height × 4` 保守估算解码内存。单张纹理解码后若超过整个预算会失败关闭，LRU 会持续淘汰直到张数和 bytes 同时合格。
- 成功图片以可释放租约交给 `AsyncCardImageView`；虚拟列表复用、解绑和销毁会归还租约。`Application.lowMemory` 立即销毁未显示纹理，仍显示的纹理只标记为“释放后删除”，不会让屏幕上的卡图突然变空；低内存前开始的加载由代际检查拒绝重新进入缓存。
- UI 取消会立即停止等待并忽略旧结果；相同图片的共享本地读取不会因一个虚拟行离开而破坏其他消费者。由于当前来源使用一次性 `File.ReadAllBytes`，已经开始的极短本地读取不强制中断，但没有消费者或低内存后的结果不会被界面采用或绕过代际保护。
- `PrivateContentImageSource` 在读取前检查文件长度，并以 32 MiB 为默认压缩文件安全上限，读取后的 race 情况再次校验；异常超大资源不会进入 Hash 或解码阶段。
- 收藏、开包和图鉴控制器均暴露当前解码 bytes 与预算上限，供 PlayMode 及后续 Unity Profiler/真机收据读取。定向 EditMode 16/16、定向 PlayMode 4/4、完整 EditMode 432/432、完整 PlayMode 11/11 通过；256 张连续加载压力项始终保持张数与 bytes 在预算内。
- 本切片没有构建 APK，也没有访问 Site、Cloudflare R2 或任何远端写入接口。中端 Android 长时运行、系统 low-memory 实机触发、触觉、音频和蜂窝网络证据仍需连接实体设备后完成，因此 4I 的“实体机”部分不宣称完成。
...
4. 再接可恢复身份。邮箱/OIDC 只申请 `openid/email/offline_access`，并使用独立游戏项目与环境。
...
4H 本地安全恢复已于 2026-07-31 完成：schema v1 envelope 使用独立游戏命名空间、来源安装 ID、UTC 时间、严格 JSON/大小/存档结构验证和 SHA-256；损坏、篡改、错误命名空间、非法收藏引用及未知字段都会在预览阶段拒绝。导入前先写原子备份，应用任一步失败会自动恢复原状态；恢复内容包含收藏、NEW、开包计数/历史/统计、独立 UI/卡片语言及声音/震动/减少动态/动画速度，不包含下载卡图、Catalog、邮箱、令牌或远端配置。设置页新增中英双语的保存、选择/预览、确认使用三步界面，确认前不可写入，并沿用按压/成功/失败音效、淡入动画及减少动态效果。
...
4H 可恢复身份基础设施已于 2026-07-31 完成：版本化配置固定本游戏 Project ID、独立 `universal-gacha-simulator/player-identity` 命名空间与 `gacha-linked` Profile，只允许 `openid/email/offline_access`，明确拒绝 Gmail、Google Drive 和额外 scope。入口在 Unity Player Accounts Client ID 为空及配置 `enabled=false` 时显示“只保存在本设备”并保持不可点击，不把匿名 Player ID 冒充成可恢复账号。

真实启用后，新邮箱身份会原地绑定当前匿名 UGS Player，不改变已有收藏；若邮箱已经属于另一名玩家，则不使用 `ForceLink`，而是先创建完整恢复备份、切换到隔离认证 Profile、读取该账号云存档，再复用显式冲突流程。双方都有不同进度时继续保留本地并要求玩家选择；读取、应用或云写入失败时恢复原本本地状态与认证 Profile。成功后只保存认证 Profile 名称，不保存完整邮箱、访问令牌或卡图；界面最多显示掩码邮箱或 Player ID 后六位。完整 EditMode 423/423、PlayMode 11/11 通过；没有重建 APK，也没有访问、上传、配置或删除 Site/R2。实际 Client ID、真实邮箱浏览器回调及换机演练仍是外部检查点，不阻塞 4J 等本机范围。
...
### 4M1 本机完成度审计基线（2026-07-31）

- `project_completion_audit.ps1` 已从旧 218 包/schema v1 默认值升级为正式 `android-complete` 538 包/revision 6/schema v2，并逐包重新验证 1.30 GB 归档的路径、大小与 SHA-256。Catalog ID 必须唯一、metadata 必须完整，旧目录或少包不能再误报通过。
...
### 4M4 Site 正式远端发行（2026-08-01）

- `Cloud/TCGContentSite` 已升级为同时接受 schema v1/v2；v2 localized name、日期、排序、tags、依赖存在性、自依赖和依赖环均执行严格验证。后端 17/17、页面/路由 5/5、typecheck、lint 与生产构建通过；正式 504,816-byte Catalog 经服务端解析再序列化后字节完全一致。
- Site version 4 已公开部署到原有稳定域名。Unity 电脑端发布器预检 538 个本机归档后，按 ZIP-first/catalog-last 顺序上传 4 个新对象、复用 534 个内容寻址对象；全部对象都完成 origin 与公网完整回读及 SHA-256 复算后，才切换 schema v2/revision 6 Catalog。
...
- 538 包及 2,000 包压力目录使用虚拟化内容库，排序、筛选、队列、空间和网络策略均通过。
...
4I 图片内存预算与 4L 本机模块化已经完成；4H 本机实现也已完成，等取得 Unity Player Accounts Client ID 后再做真实邮箱与换机验收。schema v2/revision 6 已在用户明确接受的正式 Site 托管目标完成全量只读审计，因此远端 4% 已取得。4I 的实体震动、扬声器、蜂窝和长时温度结论仍等待真实手机。4K R2 迁移保持暂停；除非用户以后明确恢复，不要求 R2 凭据，也不执行 R2 上传、配置或删除。当前 96%，在设备检查点缺失时不宣称最终平台 100%。
