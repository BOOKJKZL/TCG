...
| 切片 | 主要交付 | 验收重点 | 状态 |
|---|---|---|---|
| 4A 正式资料语义审计 | 可重复的覆盖率、重复、候选与缺失报告 | 正式 44,076 记录；报告 Hash 稳定；不写源资料 | 待开始 |
| 4B 跨语言身份系统 | Set 等价表、卡牌指纹、置信度和人工复核清单 | 高置信自动合并；歧义不合并；三种语言组合统计 | 待开始 |
| 4C 卡图补齐与容量优化 | 日文/英文缺图来源层；简中 WebP 质量实验 | Hash、来源、失败清单；视觉与容量对比；可回滚 | 待开始 |
| 4D 运行时整合与重发 | 正式跨语言组进入 Catalog，详情和图鉴真实可切换 | 正式资料抽样；单语言隐藏；收藏数量不合并 | 待开始 |
...
| 4K R2 迁移 | 私人 bucket、公开只读域名、对象复制和回滚 | 全包 HEAD/Range/Hash；8/8 写入拒绝；无密钥泄漏 | 待开始 |
...
- 未匹配不等于失败；每条记录必须被分类为“已匹配、语言独占、缺少对应资料、待复核或明确拒绝”。
...
下一次用户说“开始”或“继续”时，从 4A 正式资料语义审计开始；在本机切片全部完成前不要求 R2 凭据。
