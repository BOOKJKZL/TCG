...
这些模型已经支撑五个本机系列、1462 个运行时 Printing、三套历史规则和两套来源指导模拟。阶段 6A–6C5 已加入安装决策、安全路径、本地收据、可回滚 ZIP 安装/卸载、下载状态机、文件断点缓存、严格 HTTP Range、版本化 catalog、协调器，以及带动画、音效、本地化和主线程派发的玩家内容管理页面；卸载只触及收据登记内容，收藏存档保持独立，并允许同页重装。阶段 7A 已加入受限 HTTPS catalog 与私人配置，7B 已加入确定性 ZIP/catalog 发布和运行时安装自验证，7C 已加入私人 R2 上传边界，7D 已加入来源绑定的已验证 catalog 离线缓存和跨重启续传，7E 已完成临时 Site R2 中继、电脑直传适配器、owner-only 凭据配对及两个真实卡包的公网发布；浏览器不再接触本机卡包。阶段 8A 已将规则来源升级为 Application 证据模型；8B 已让五个系列分别使用可替换规则、独立主题、原创包装、有界粒子与正式烘焙音效。第一大阶段的软件与 Android 模拟器验收已经关闭；第二大阶段 Phase 2A 已固定 Set 排序、Manifest v2 与形态政策边界，Phase 2B 已加入只读 metadata inventory、稳定内容 Hash 和容量抽样。下一步是只写本机私人目录的可恢复英文批量导入；独立 Cloudflare R2 仍延后为存储适配器迁移。
