...
本次修改原因：第一大阶段的软件范围已经完成 100% 验收；第二大阶段 Phase 2A–2E 也已完成，使总计划进入 57%。218 个英文 Set 已形成确定性不可变运行时包并通过全量离线安装回读；PokéAPI 资料层已固定 9 世代、1,025 物种、1,351 个具体变体与 1,579 个形态，Gen 1 精确为 #001–#151。最新回归为 PlayMode 7/7、EditMode 280/280。实体震动手感、扬声器音质和真实蜂窝切换继续作为实体机补测限制。验证节奏固定为 Play Mode 优先，普通数据与代码变化不重复构建 APK，只有权限、ABI、存储、图形、触觉等平台边界才构建。
...
- 当前核心 EditMode 280/280、PlayMode 7/7 全部通过。
...
→ 当前下一步：Phase 2F Card→Species/Form 多对多关联器
```

Phase 2E 已完成 generation/species/form snapshot、本地化 fallback、形态分类与全量审计，手机端仍不直接依赖 PokéAPI。下一步从 23,444 张英文卡牌生成稳定的物种/形态关联与人工复核队列；全量远端 Catalog 在图鉴与关联包完成后的 Phase 2J 一次发布。
