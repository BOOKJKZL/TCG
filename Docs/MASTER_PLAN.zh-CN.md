...
→ 第四阶段计划：资料语义、内容体验、存档恢复、R2、实体机与模块化（4A–4C 完成；4K R2 暂停）
→ 当前下一步：4D 精简跨语言覆盖包的本地内容寻址重发与回读验证
```
...
第四阶段 4D1 正式身份运行时整合已于 2026-07-31 完成：4B 报告被确定性编译为 147 组/294 成员/84,777 bytes 的严格玩家覆盖文件，两次 SHA-256 均为 `3568e20cdf3d202867c2739c1ded291911c1d67c72d5f1a4ee8adbef066cedce`。正式 Catalog 让 147/147 组进入运行时，同时保留 44,076 个独立来源 Item 与 53,296 个 Printing；只有显式接受组可以切换，待复核英文/简中同号碰撞保持单语言。收藏数量仍按 Printing 保存，卡图/名称/地区 Set/卡号/稀有度/variant 同步切换，应用语言和全局卡片语言不被详情切换修改，并保留 120 ms 淡入、翻卡音效与减少动态效果。完整 EditMode 363/363、PlayMode 10/10 通过；4D 下一步为本地内容寻址重发与回读，不上传 Site/R2。
