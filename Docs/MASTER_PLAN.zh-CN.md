...
本次修改原因：第一、第二大阶段的软件范围均已完成 100% 验收。英文 218 Set、23,444 卡、9 世代、1,025 物种、1,579 形态、23,444 卡牌关联与 1,571 张图鉴图已组成 229 个按需资源包并发布到只读 Site；完整 EditMode 299/299、PlayMode 8/8、远端 229/229 协议审计、最终 ARM64 APK 和 Android 14 安装收据全部通过。实体震动手感、扬声器音质和真实蜂窝切换继续作为实体机体验补测限制，不影响软件范围结论。验证节奏固定为 Play Mode 优先，只有权限、ABI、存储、图形、触觉等平台边界变化才构建 APK。
...
→ 当前下一步：建立第三阶段计划（多语言/历史配列/独立 R2/实体设备体验，按需选择）
```

Phase 2A–2J 已完成。手机端只读取已发布的内容寻址 catalog/ZIP，不直接请求 TCGdex、PokéAPI 或 GitHub；电脑端导入、转换、压缩、Hash 与发布继续保持私人写入边界。

第三阶段已于 2026-07-30 开始，范围固定为原创训练家终端 UI、Android 顺畅度，以及英文 `en`、日文 `ja`、简体中文 `zh-cn` 三种实体卡牌版本。跨语言卡牌采用独立版本组：只有一个已安装版本时直接显示；有两个或三个版本时才显示语言切换，并同时更换卡图与全部印刷资料。容量基线、数据身份、性能预算、分片与验收规则见 `PHASE_3_UI_PERFORMANCE_MULTILINGUAL_CARDS.zh-CN.md`。Cloudflare R2 迁移和实体硬件体验仍是独立后续范围，不能冒充本阶段已经验证的结果。
