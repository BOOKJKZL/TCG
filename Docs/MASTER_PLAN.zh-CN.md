...
- 反馈系统、通用领域模型、Application 状态、内容适配器、图片源、纹理缓存和新抽卡引擎均有自动化测试；当前项目 EditMode 测试为 36/36 通过。
...
- R2 上传与手机远程下载闭环。
- 宝可梦不同年代的真实卡包配列规则。
- 开包 PlayMode 流程和 Android 真机验证。

按验收条件而不是代码数量估算，当前技术底座约完成 70%，本地通用模拟器 MVP 约完成 55%，完整计划约完成 35%。
...
状态：存档、统一反馈和已安装内容浏览基础已完成；收藏库存语义、产品选择、开包演出和结果页尚未形成闭环。
...
- 已完成系列/全卡资料浏览、虚拟化图片加载和详情基础；待把库存数量、新卡、重复卡、搜索和筛选接入同一界面。
