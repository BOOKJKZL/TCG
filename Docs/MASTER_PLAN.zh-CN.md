...
- 反馈系统、通用领域模型、内容适配器和新抽卡引擎均有自动化测试；当前项目 EditMode 测试为 16/16 通过。
...
- R2 上传与手机远程下载闭环。
- 宝可梦不同年代的真实卡包配列规则。
- 独立的 Application 层；当前 `GachaViewController` 仍直接调用 Infrastructure。
- PlayMode 测试和 Android 真机验证。

按验收条件而不是代码数量估算，当前技术底座约完成 60%，本地通用模拟器 MVP 约完成 45%，完整计划约完成 25%–30%。

## 四、阶段计划

### 阶段 0：工程与模块整理

状态：基础切片已完成（2026-07-16）；Application 边界仍待补齐，后续模块会在迁移时逐步增加各自 asmdef。
...
状态：领域模型和旧 UI Locale 切换具备部分基础；双服务、回退、持久化和设置界面尚未完成。
