...
public static class LanguageLocalizationSeeder
{
    ...
    public static void Seed()
    {
        ...
        Apply(english, new Dictionary<string, string>
        {
            ...
            ["collection.action.filters"] = "Filters and sorting",
            ["collection.filters.title"] = "Filter cards",
            ...
            ["collection.sort.number"] = "Card number",
            ...
            ["gacha.pack.interaction.mobile"] = "Drag to rotate. Pull apart with two fingers to tear.",
            ["gacha.pack.interaction.desktop"] = "Drag to rotate. Pull outward from the center seam to tear.",
            ["gacha.pack.selection"] = "Pack {0} of {1} selected",
            ...
            ["settings.subtitle"] = "Languages, comfort, downloads, saves, and account safety",
            ["settings.value"] = "{0}: {1}",
            ["settings.language.description"] = "Interface language changes menus only. Card language controls which card data you use.",
            ["settings.language.status.ready"] = "Interface and card languages are saved independently.",
            ["settings.language.status.saved"] = "Language preference saved.",
            ["settings.language.status.failed"] = "The language preference could not be saved.",
            ["settings.experience.description"] = "Choose the feedback and animation level that feels comfortable.",
            ["settings.experience.save_failed"] = "The preference could not be saved. Nothing changed.",
            ["settings.experience.save_failed_safe"] = "The preference could not be saved. Nothing changed.",
            ["settings.download.title"] = "Downloads",
            ["settings.download.description"] = "Large downloads can wait for Wi-Fi. Every download still shows a confirmation first.",
            ["settings.download.status.ready"] = "This policy is shared with the Content Library.",
            ["settings.download.status.saved"] = "Download policy saved.",
            ["settings.download.status.failed"] = "The download policy could not be saved. Nothing changed.",
            ["settings.recovery.status.exported_safe"] = "Verified backup exported.",
            ["settings.recovery.status.exported"] = "Verified backup exported.",
            ["settings.recovery.status.error"] = "Recovery could not continue. Your current progress was not changed.",
            ["settings.recovery.status.imported"] = "Import complete. A safety backup was created first.",
            ["settings.recovery.status.error_safe"] = "Recovery could not continue. Your current progress was not changed.",
            ["settings.recovery.status.imported_safe"] = "Import complete. A safety backup was created first.",
            ["settings.recovery.confirm.title"] = "Replace current progress?",
            ["settings.recovery.confirm.body"] = "The selected backup is verified. A safety backup of current progress will be created before import.",
            ["settings.account.title"] = "Account and cloud",
            ["settings.account.description"] = "A recoverable PLAYER ID is separate from anonymous cloud service availability.",
            ["settings.cloud.status.resolved_safe"] = "Conflict resolved. A safety backup was created first.",
            ["settings.cloud.status.resolved"] = "Conflict resolved. A safety backup was created first.",
            ["settings.cloud.status.failed"] = "Nothing changed. Review both saves and try again.",
            ["settings.cloud.status.backup_failed"] = "A safety backup could not be created, so nothing changed.",
            ["settings.cloud.status.failed_safe"] = "Nothing changed. Review both saves and try again.",
            ["settings.cloud.status.backup_failed_safe"] = "A safety backup could not be created, so nothing changed.",
            ["settings.cloud.confirm.title"] = "Confirm save choice",
            ["settings.cloud.confirm.body"] = "This choice updates local and cloud progress only after a safety backup succeeds.",
            ["settings.identity.status.cloud_pending_safe"] = "PLAYER ID is ready, but initial cloud sync is still pending. Your local progress remains available.",
            ["settings.identity.status.cloud_pending"] = "PLAYER ID is ready, but initial cloud sync is still pending. Your local progress remains available.",
            ["settings.identity.status.failed"] = "PLAYER ID was not changed. Your local progress remains available.",
            ["settings.identity.status.failed_safe"] = "PLAYER ID was not changed. Your local progress remains available."
        });
        ...
        Apply(chinese, new Dictionary<string, string>
        {
            ...
            ["collection.action.filters"] = "筛选与排序",
            ["collection.filters.title"] = "筛选卡牌",
            ...
            ["collection.sort.label"] = "排序",
            ["collection.sort.number"] = "卡牌编号",
            ["collection.sort.name"] = "名称",
            ...
            ["gacha.pack.interaction.mobile"] = "移动卡包查看前面、后面；用2指分开卡包。",
            ["gacha.pack.interaction.desktop"] = "移动卡包查看前面、后面；在中间分开卡包。",
            ["gacha.pack.selection"] = "已选择第 {0} / {1} 包",
            ...
            ["home.nav.home"] = "主菜单",
            ["settings.title"] = "设置",
            ["settings.subtitle"] = "语言、舒适度、下载、存档与账号安全",
            ["settings.value"] = "{0}：{1}",
            ["settings.language.description"] = "界面语言只改变菜单文字；卡牌语言决定使用哪种卡牌内容。",
            ["settings.language.status.ready"] = "界面语言和卡牌语言会分别保存。",
            ["settings.language.status.saved"] = "语言偏好已保存。",
            ["settings.language.status.failed"] = "无法保存语言偏好。",
            ["settings.experience.description"] = "选择适合你的声音、触感和动画强度。",
            ["settings.experience.save_failed"] = "无法保存此偏好，设置未改变。",
            ["settings.experience.save_failed_safe"] = "无法保存此偏好，设置未改变。",
            ["settings.download.title"] = "下载",
            ["settings.download.description"] = "大型下载可以等待 Wi-Fi；每次下载仍会先显示确认。",
            ["settings.download.status.ready"] = "此政策与内容库共用。",
            ["settings.download.status.saved"] = "下载政策已保存。",
            ["settings.download.status.failed"] = "无法保存下载政策，设置未改变。",
            ["settings.recovery.status.exported_safe"] = "验证备份已导出。",
            ["settings.recovery.status.exported"] = "验证备份已导出。",
            ["settings.recovery.status.error"] = "无法继续恢复，当前进度未改变。",
            ["settings.recovery.status.imported"] = "导入完成，并已先创建安全备份。",
            ["settings.recovery.status.error_safe"] = "无法继续恢复，当前进度未改变。",
            ["settings.recovery.status.imported_safe"] = "导入完成，并已先创建安全备份。",
            ["settings.recovery.confirm.title"] = "替换当前进度？",
            ["settings.recovery.confirm.body"] = "所选备份已通过验证。导入前会先备份当前进度。",
            ["settings.account.title"] = "账号与云端",
            ["settings.account.description"] = "可恢复的 PLAYER ID 与匿名云服务状态是两件不同的事。",
            ["settings.cloud.status.resolved_safe"] = "冲突已解决，并已先创建安全备份。",
            ["settings.cloud.status.resolved"] = "冲突已解决，并已先创建安全备份。",
            ["settings.cloud.status.failed"] = "未进行任何更改。请检查两份存档后重试。",
            ["settings.cloud.status.backup_failed"] = "无法创建安全备份，因此未进行任何更改。",
            ["settings.cloud.status.failed_safe"] = "未进行任何更改。请检查两份存档后重试。",
            ["settings.cloud.status.backup_failed_safe"] = "无法创建安全备份，因此未进行任何更改。",
            ["settings.cloud.confirm.title"] = "确认存档选择",
            ["settings.cloud.confirm.body"] = "只有成功创建安全备份后，才会更新本机与云端进度。",
            ["settings.identity.status.cloud_pending_safe"] = "PLAYER ID 已就绪，但首次云端同步仍在等待。本机进度仍可使用。",
            ["settings.identity.status.cloud_pending"] = "PLAYER ID 已就绪，但首次云端同步仍在等待。本机进度仍可使用。",
            ["settings.identity.status.failed"] = "PLAYER ID 未改变。本机进度仍可使用。",
            ["settings.identity.status.failed_safe"] = "PLAYER ID 未改变。本机进度仍可使用。"
        });
